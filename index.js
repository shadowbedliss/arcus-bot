...
function buildOperationEmbed(op) {
  return new EmbedBuilder()
    ...
    .setFooter({ text: `ID: ${op.id} | Creator: ${op.creatorTag}` });
}
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.options.getSubcommand() === 'settings') {
        ...
        ); // FIX: Ensure row2 is correctly defined
    ...
    if (parts[1] === 'attendance') {
      ...
      for (const participant of currentOp.participants) {
        ...
        userStats.joined += 1;
        if (attended) userStats.attended += 1; // You could weight "Late" differently here if needed
      }
